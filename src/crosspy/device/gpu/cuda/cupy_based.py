...
class CuPyCarrier:
    ...
    def pull(self, src, out=None, copy=False):
        ...
        if isinstance(src, cupy.ndarray):
            ...
            return buf
        if isinstance(src, cupyx.scipy.sparse.spmatrix):
            if not isinstance(src, cupyx.scipy.sparse.coo_matrix):
                raise NotImplementedError
            if copy:
                raise NotImplementedError
            src.data = self.pull(src.data)
            src.row = self.pull(src.row)
            src.col = self.pull(src.col)
            return src
        try:
