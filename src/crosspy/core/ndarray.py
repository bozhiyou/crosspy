...
from crosspy.core.indexing import Index, BasicIndexType, IndexType
from .utils import is_empty_slice, shape_of_slice
from crosspy.utils.shape import ShapeType, compose_shape
...
class Initializer:
    ...
    def __init__(self, obj, axis: Optional[int] = None, *, data_manager = None) -> None:
        """
        Compose objects:
            take care of shapes based on `axis`
        Case 0 - singletons:
            partition?
        Case 1:
            list/tuple of objs
        ...
        self._shape = getattr(self._original_data, 'shape', compose_shape(shapes, self._concat_axis))
...
class CrossPyArray(numpy.lib.mixins.NDArrayOperatorsMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.device_array: dict = device_array

        self.data_manager = data_manager
        self._original_data = original_data  # NOTE this holds reference to the original data, which may or may not be expected

        from crosspy.transfer.collective import DeviceProxy
        self.device_proxies = [DeviceProxy(did, get_device(darray)) for did, darray in self.device_array.items()]
    ...
    def _global_index_to_block_id(self, i, out=None, stream=None):
        ...
        with context(i, stream=stream) as ctx:
            block_offset_ = ctx.pull(self._metadata.block_offset, copy=False)
    ...
    def __getitem__(self, index: IndexType):  # -> Union[Array, List[Array]]
        ...
        index = Index.parse(index, self.shape)
        ...
        if self._concat_axis == 0:
            if isinstance(index[self._concat_axis], slice):
                ...
                return CrossPyArray((metadata.block_offset[-1],), self._dtype, metadata, device_array, axis=self._concat_axis)
            if is_array(index[0]) and hasattr(index[0], 'device'):
                # TODO deal with crosspy array (many devices)
                addr = index[0]
                import asyncio
                async def transfer():
                    ...
                    await asyncio.gather(
                            _endpoint_setup(self.device_proxies, self.metadata, addr)
                        )
            ...
            if isinstance(index[self._concat_axis], int) or is_array(index[0]) and index[0].ndim == 0:
                int_ = index[self._concat_axis]
                blkid = self._global_index_to_block_id(int_)
                with context(blkid) if self.data_manager is None else self.data_manager.get_device(blkid) as ctx:
                    d = ctx.pull(self._metadata.device_idx)[blkid]
                    i = int_ - ctx.pull(self._metadata.device_offset)[blkid]
                    da = self.device_array[int(d)]
                    with context(da) as actx:
                        selection_ = da[actx.pull(i)]
                    selection = ctx.pull(selection_)
