...
def main(args, T):
    ...
    for i in range(args.num_gpus):
        with cp.cuda.Device(0) as dev:
            random_array = cp.random.randint(0, 10000, size=args.m)
            random_array = random_array.astype(cp.int32)

            cupy_list_A.append(random_array)
            cupy_list_B.append(cp.zeros(args.m, dtype=cp.int32))
            dev.synchronize()
    ...
    with cp.cuda.Device(0) as d:
        quicksort(size_prefix, A, workspace, 0, args.m*args.num_gpus, T)
        d.synchronize()
